...
var OPENAI_COMPLETIONS_URL = `https://api.openai.com/v1/chat/completions`;
var CHAT_MODELS = {
  GPT35: {
    name: "gpt-3.5-turbo",
    tokenLimit: 4096
...
async function getChatGPTCompletion(apiKey, apiUrl, model, messages, settings) {
  var _a, _b, _c;
  const headers = {
...
var SettingsTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian2.Setting(containerEl).setName("System prompt").setDesc(`The system prompt sent with each request to the API. 
(Note: you can override this by beginning a note stream with a note starting 'SYSTEM PROMPT'. The remaining content of that note will be used as system prompt.)`).addTextArea((component) => {
    ...
    new import_obsidian2.Setting(containerEl).setName("Temperature").setDesc("Sampling temperature (0-2). 0 means no randomness.").addText(
    ...
    new import_obsidian2.Setting(containerEl).setName("API URL").setDesc(
      "The chat completions URL to use. You probably won't need to change this."
    ).addText(
      (text) => {
        text.inputEl.style.width = "300px";
        text.setPlaceholder("API URL").setValue(this.plugin.settings.apiUrl).onChange(async (value) => {
...
var createNode = (canvas, parentNode, nodeOptions, nodeData) => {
  ...
  const siblingsRight = (siblings == null ? void 0 : siblings.length) ? siblings.reduce(
